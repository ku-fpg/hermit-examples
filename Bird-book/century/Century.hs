...
{-# RULES "map-fusion-eta" [~] forall f g xs.  map (f . g) xs = (map f . map g) xs  #-}

{-# RULES "map-id"        [~]               map id      = id   #-}
{-# RULES "map-id-eta"    [~] forall xs.    map id xs   = xs   #-}
...
{-# RULES "6.9-eta" [~] forall f g xs.  map (fork (f , g)) xs   =  zip (map f xs , map g xs)    #-}

{-# RULES "fork-fst-snd" [~]  fork (fst , snd)  =  id  #-}
{-# RULES "zip-unzip"    [~]  zip . unzip  =  id       #-}

{-# RULES "6.10"      [~] forall f g p.      map (fork (f,g)) . filter (p . g)     =  filter (p . snd) . map (fork (f,g))     #-}
{-# RULES "6.10-eta"  [~] forall f g p xs.   map (fork (f,g)) (filter (p . g) xs)  =  filter (p . snd) (map (fork (f,g)) xs)  #-}
